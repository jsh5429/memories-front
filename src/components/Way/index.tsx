...
import { GetWayResponseBodyDto } from "src/apis/dto/response/openai";
...
function Way({ type } : Props) {
    ...
    const accesToken = cookies[ACCESS_TOKEN];

    // function : get way response 처리 함수 //
    const getWayResponse = (responseBody : GetWayResponseBodyDto | ResponseDto | null) => {
        const message = 
        ...
        const { result } = responseBody as GetWayResponseBodyDto;
        setResult(result);
    }
    ...
    useEffect(()=>{
        ...
        getWayRequest(requestBody, accesToken).then(getWayResponse);
    },[])
